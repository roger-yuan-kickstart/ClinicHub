...
STORY-003 (日志工具)
    ...
    └──→     STORY-005 ✅（截图工具，PR 已合并）
         ↓
STORY-006 ✅（类型定义，`src/types/index.ts`）
    ↓
...
| STORY-005 | 截图工具模块 | Agent | `[x] 已完成` | PR #7 已合并；`screenshot.ts`（`fullPage` + 结构化 info 日志）；依赖 003；**下一步 → 007** |
| STORY-006 | 共享类型定义 | Agent | `[x] 已完成` | `src/types/index.ts`；`TaskConfig` / `config.ts` 对齐；依赖 002；**下一步 → 007** |
