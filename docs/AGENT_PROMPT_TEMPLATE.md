...
## PR 审查后的修复派遣（Top Agent → Sub Agent）

当 Top Agent 完成 **PR / diff 的 code review**，需要再派 Implementation Agent **在同一分支上修 PR**（而不是开新 Story）时，除了照常说明要改的文件与验收点外，**必须在 Prompt 末尾附带一段标准化的审查结论**，方便执行者判断优先级与合并门槛。

### 必须包含：`Overall score`（0–100）

在审查结论的**第一行**给出总分，语义约定如下：

| 分数区间 | 含义（给 Sub Agent 的解读） |
|---|---|
| **90–100** | 可合并：仅剩可选 nit，或文档/措辞类小改 |
| **75–89** | 质量良好：有明确应改项，但无架构级硬伤 |
| **60–74** | 需实质修改：缺失 AC、明显技术债或一致性问题 |
| **0–59** | 需较大返工：方向错误、重大安全隐患或大面积不符合项目约束 |

**约定：** **100 = 满分**，**0 = 最低**；分数与「是否值得合并」挂钩，不是人情分。

### 建议结构（复制到 Prompt 末尾）

```markdown
## PR review handoff（Top Agent）
...
### Verification
- 修完后：`pnpm typecheck`、`pnpm lint`（以及 PR 中已列出的其他命令）
```

Sub Agent 交付时，可在自检报告中**回应每一项**（已修 / 不修及理由），便于 Top Agent 二次 review。
...
- [ ] **（若是修 PR）有 Overall score 与分级清单**：含 0–100 总分及 Must fix / Should fix / Nit（见上文「PR 审查后的修复派遣」）
